...
<body>
            ...
            <div class="card-body">
                <div class="form-group row">
                    ...
                    <div class="col-sm-4">
                        <input type="text" class="form-control" id="id_pembelian" name="id_pembelian"
                            value="<?= $pembelian ? $pembelian->id_pembelian : "" ?>" readonly>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-4">                                                                                  
                        <input type="date" class="form-control" id="tanggal" name="tanggal"
                            value="<?= $pembelian ?  $pembelian->tanggal : "" ?>"
                            required>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-4">
                        <select name="id_supplier" class="form-control" required>
                            <option value="" disabled <?php if (empty($pembelian)) : ?>selected<?php endif ?>>
                                Pilih Data Supplier!</option>
                            <?php foreach ($supplier as $spl) : ?>
                            <?php if (!empty($pembelian)) : ?>
                            <?php $selected = $spl->id_supplier == $pembelian->id_supplier ? "selected" : ""; ?>
                            <?php else : ?>
                            <?php $selected = ""; ?>
                            <?php endif; ?>
                            <option value="<?= $spl->id_supplier ?>" <?= $selected ?>><?= $spl->supplier ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
                ...
                <table id="purchaseTable" class="table table-bordered table-hover dt-responsive" style="width: 100%">
                    ...
                    <tbody id="purchaseItems">
                        <?php if (!empty($pembelian)) : ?>
                        <?php foreach ($detail_pembelian->getData("tb_detail_pembelian", ["id_pembelian" => $pembelian->id_pembelian])->result() as $detail) : ?>
                        <?php $num = 1; ?>
                        <?php $total += $detail->jumlah * $detail->harga_beli; ?>
                        <tr data-iddetail="<?= $detail->id_detail_pembelian ?>" data-id="<?= $detail->id_obat ?>" data-rownum="<?= $num++ ?>">
    ...
    $("body").on("submit", "#purchaseForm", function(e) {
        ...
        const urlForm = data.id_pembelian.length == 0  ? "http://localhost/skripsi/master/pembelian/tambah":"http://localhost/skripsi/master/pembelian/update";
